...
describe("player-record migration", () => {
  ...
  it("keeps valid daily puzzle outcomes while dropping malformed legacy data", () => {
    const migrated = migratePlayerRecords({
      ...
      dailyCountry: {
        outcomes: {
          "2026-07-27": { status: "solved", guessesUsed: 2, guesses: ["France", "", "x".repeat(81)], selectedClueIds: ["location", "language", "not-a-clue", "capital", "geography"] },
    ...
    expect(migrated.dailyCountry).toEqual({
      outcomes: {
        "2026-07-27": { status: "solved", guessesUsed: 2, guesses: ["France"], selectedClueIds: ["location", "language"] },
      },
    });
  });
