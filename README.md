...
When Analytics is accepted, PostHog EU receives explicit page, game, campaign, and Web Vitals events with an ephemeral pseudonymous visitor ID. Autocapture, automatic page-view capture, campaign/referrer capture, feature flags, session recording, heatmaps, console/network recording, `identify`, and known-person profiles are disabled. Puzzler emits its own sanitised `page_view` event and enables only LCP, CLS, FCP, and INP Web Vitals with attribution and network timing disabled. PostHog runs in memory-only mode with provider persistence disabled, and a final `before_send` allowlist keeps only the required `token`, `distinct_id`, and `$process_person_profile: false` transport fields alongside Puzzler&apos;s declared event properties. It removes SDK-added full URLs, referrers, device, browser, and session data. When Marketing is accepted, the Meta Pixel receives deliberate `PageView` events and one `FirstGameCompleted` custom conversion while Marketing consent remains granted. Automatic advanced matching is not used. Rejecting non-essential cookies loads neither service. Preferences can be changed or withdrawn from the persistent **Cookie settings** footer control.

### Event taxonomy

PostHog uses these snake-case events: `page_view`, `ad_landing_viewed`, `game_selected`, `game_started`, `game_completed`, `game_abandoned`, `replay_started`, and `first_game_completed`. It also records the PostHog `$web_vitals` event with only numeric LCP, CLS, FCP, and INP values.
...
2. For PostHog, use Live Events and verify the explicit event name, documented event properties, `token`, a non-empty `distinct_id`, and `$process_person_profile: false`. Confirm that `page_view` contains only `page_path`, `$web_vitals` contains only numeric LCP/CLS/FCP/INP values, and `$current_url`, referrer, device, browser, and session properties are absent. Rejecting Analytics must cause no further events.
